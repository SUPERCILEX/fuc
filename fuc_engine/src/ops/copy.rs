...
mod compat {
        ...
        for Impl<Params, LF>
    {
        ...
        fn run(&self, (from, to): (Cow<Path>, Cow<Path>)) -> Result<(), Error> {
            ...
            if hard_link {
                spawner.spawn1(|spawner, state| copy_dir::<true>(node, state, spawner));
            } else {
                spawner.spawn1(|spawner, state| copy_dir::<false>(node, state, spawner));
            }
    ...
    fn copy_dir<const HARD_LINK: bool>(
        ...
        loop {
            ...
            if file_type == FileType::Directory {
                ...
                tasks.spawn1(|spawner, state| copy_dir::<HARD_LINK>(node, state, spawner));
            } else {
                tasks.flush();
                if HARD_LINK {
                    let name = file.file_name();
